 ...
 *
 * This is the base repository class for ticket entities.
 */
class Ticket extends EntityRepository
{
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('index', 'view'))) {
                ...
                $listHelper = new ListEntriesUtil(ServiceUtil::getManager(), ModUtil::getModule('GuiteKnowledgeBaseModule'));
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('ticket', 'workflowState');
            }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('GuiteKnowledgeBaseModule', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'ticket', 'source' => 'GET'));
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Guite\KnowledgeBaseModule\Entity\TicketEntity', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Guite\KnowledgeBaseModule\Entity\TicketEntity', 'tbl')
     ...
     *
     * @return array|Guite\KnowledgeBaseModule\Entity\TicketEntity retrieved data array or Guite\KnowledgeBaseModule\Entity\TicketEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Guite\KnowledgeBaseModule\Entity\TicketEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('GuiteKnowledgeBaseModule', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'ticket', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'GuiteKnowledgeBaseModule') {
            return $qb;
        }
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= 'tbl.subject LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.content LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.subject LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.ratesUp = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.ratesDown = \'' . $fragment . '\'';
        }
        $where = '(' . $where . ')';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numTickets';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Guite\KnowledgeBaseModule\Entity\TicketEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of tickets to exclude (optional).
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $categoryProperties = ModUtil::apiFunc('GuiteKnowledgeBaseModule', 'category', 'getAllProperties', array('ot' => 'ticket'));
        foreach ($categoryProperties as $propertyName => $registryId) {
            $config['plugins'][] = new CategoryFilter('GuiteKnowledgeBaseModule', $propertyName, 'categories' . ucwords($propertyName));
        }
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('GuiteKnowledgeBaseModule', 'showOnlyOwnEntries', 0), 'GETPOST');

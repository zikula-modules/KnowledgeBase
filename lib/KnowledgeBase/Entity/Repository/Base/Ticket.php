 ...
 */
class KnowledgeBase_Entity_Repository_Base_Ticket extends EntityRepository
{
    /**
    ...
    public function truncateTable()
    {
        $query = $this->getEntityManager()
                 ->createQuery('DELETE KnowledgeBase_Entity_Ticket');
        $query->execute();
    }
     ...
     *
     * @return array|KnowledgeBase_Entity_Ticket retrieved data array or KnowledgeBase_Entity_Ticket instance
     */
     ...
     * @param boolean $useJoins Whether to include joining related objects (optional) (default=true).
     *
     * @return ArrayCollection collection containing retrieved KnowledgeBase_Entity_Ticket instances
     */
    ...
    public function selectSearch($fragment = '', $exclude = array(), $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        $whereSub = '';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.subject LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.content LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.views = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.ratesUp = \'' . $fragment . '\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.ratesDown = \'' . $fragment . '\'';
